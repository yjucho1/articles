...
<img src='Fig1.png' width=300></img>
...
<img src='Fig3.png' width=300></img>
...
<img src='Fig2.png' width=300></img>
...
<img src='Fig4.png' width=300></img>
...
<img src='Fig5.png' width=300></img>
...
<img src='Fig6.png' width=300></img>
...
<img src='Fig7.png' width=300></img>
...
<img src='Table1.png' width=300></img>

* 딥러닝 모델이 다른 입력데이터더라도 특정 물체의 공간적 특성(예를들어 얼굴에서 눈, 코, 입의 위치와 구성)을 이해하고 있는지 알아보기 위해 5개의 개 사진에서 모두 동일하게 특정 부분(왼쪽 눈)을 가린 것과 그러지 않은 것을 비교해보았다.
* 가려진 이미지에서 계산된 피쳐벡터와 원래 이미지에서 계산된 피쳐벡터를 빼서 ε값을 구한 후, 각 이미지들의 ε값의 Hemming distance를 구한다. 
* distance 값이 낮을수록 특정 부분을 가린 것이 서로 다른 이미지에 대해서 일관된 변화를 가져왔다는 것을 의미한다. 
* Table1 결과는 이미지의 일부를 랜덤하게 가리는 것보다 모두 동일하게 왼쪽눈, 오른쪽눈, 코를 가리는 각 각의 경우가 더 낮은 Hemming distance를 보였기때문에 서로 다른 입력이미지더라도 물체의 공간적 특징을 잘 인식하고 있음을 보여준다. 

### Experiment

<img src='Table2.png' width=300></img>

* 앞서 설명한 필터사이즈와 스트라이드를 조절한 모델을 사용하여  14.7% error rate를 보였고, 15.3%의 Alexnet보다 더 우수한 성능을 보였다.

<img src='Cal101.png' width=300></img><img src='Cal256.png' width=300></img><img src='PASCAL2012.png' width=300></img>

* 또한 ImageNet 데이터를 이용해 학습한 모델을 pre-trained model로 사용하여 Caltech-101, Caltech-256, PASCAL VOC 2012 데이터에 전이학습하여 Feature generalization을 확인하였다.

### Discussion

* 이 논문에는 convnet을 이해하기 위해 각 레이어의 엑티베이션값을 시각화하는 방법을 제안하였다. 
* 이를 통해 모델이 물체의 공간적 특징을 이해하고 레이어를 거칠수록 불변하는 정보를 학습한다는 것을 나타냈다. 
* Alexnet 등 기존 모델을 개선하고 디버깅하기 위해 이러한 시각화 기법을 사용할수 있다는 것을 실험하였다. 
* ImageNet 모델을 pre-train model로 다른 데이터에 잘 일반화 할 수있음을 보였다. 